...
async def get_all_task(uid, **kwargs):
	...
	# 世界boss自动通关
	kwargs.update({"tid": enums.Task.PASS_WORLD_BOSS, "task_value": 1})
	await record_task(uid, **kwargs)
...
async def record_task(uid, **kwargs):
	data = await common.execute(
		f'INSERT INTO task (uid, tid, value,reward,timer) VALUES ("{uid}", {kwargs["tid"]}, {kwargs["task_value"]},0,"{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}") ON DUPLICATE KEY UPDATE `value`= {kwargs["task_value"]}',
		**kwargs)
