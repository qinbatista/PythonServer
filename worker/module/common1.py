...
async def _decrease_energy(uid, amount, sql_energy, max_energy, **kwargs) -> dict:
	...
	tim = now100 if tim is None else tim
	if tim > now:
		"""此时 sql_energy == _max_energy 成立，即能力无消耗，满能量状态"""
		if amount == 0:  # 获取能量
		...
		if sql_energy >= max_energy:  # 能量超出满能力状态时，不计算恢复时间
			tim, cooling = now + timedelta(days=36500), -1
		if sql_energy < 0: return mt(98, 'Not enough energy consumption')
		await set_timer(uid, enums.Timer.ENERGY_RECOVER_TIME, now, **kwargs)
		await set_progress(uid, 'energy', sql_energy, **kwargs)
		...
		"""此时 sql_energy != _max_energy 成立，即能力已消耗，恢复能量状态"""
		delta_time = now - tim
		_energy = int(delta_time.total_seconds()) // _cooling  # 计算恢复能量的点数
		if amount == 0:  # 获取能量
			# 成功3：如果有恢复时间且是获取能量值，则加上获取的能量值，并判断能量值是否满足上限
			# 满足上限的情况：直接将满能量值和空字符串分别存入能量值项和恢复时间项
			_energy += sql_energy
			tim, sql_energy, cooling = (now100, max_energy, -1) if _energy > max_energy else (tim, _energy, int(delta_time.total_seconds()) % _cooling)
			if sql_energy + _energy >= max_energy:
				recover_time, sql_energy, cooling_time = "", max_energy, -1
				await execute_update(f'UPDATE progress SET energy = {sql_energy} WHERE uid = "{uid}";', **kwargs)
				...
				return mt(4, 'Energy has been fully restored, successful energy update', {'energy': sql_energy, 'recover_time': recover_time, 'cooling_time': cooling_time})
			# 成功4：如果有恢复时间且是获取能量值，则加上获取的能量值，并判断能量值是否满足上限
			# 不满足上限的情况：将能恢复的能量值计算出来，并且计算恢复后的能量值sql_energy
			# 和恢复时间与恢复能量消耗的时间相减的恢复时间值
			else:
				recover_time, sql_energy = (datetime.strptime(recover_time, '%Y-%m-%d %H:%M:%S').replace(tzinfo=TZ_SH) + timedelta(seconds=recovered_energy * _cooling)).strftime("%Y-%m-%d %H:%M:%S"), sql_energy + recovered_energy
				delta_time = datetime.strptime(current_time, '%Y-%m-%d %H:%M:%S') - datetime.strptime(recover_time, '%Y-%m-%d %H:%M:%S')
				cooling_time = _cooling - int(delta_time.total_seconds())  # 计算下一点能量需要多少的恢复时间
				await execute_update(f'UPDATE progress SET energy = {sql_energy} WHERE uid = "{uid}";', **kwargs)
				await execute_update(f'UPDATE timer SET time = "{recover_time}" WHERE uid = "{uid}" AND tid = "{enums.Timer.ENERGY_RECOVER_TIME.value}";', **kwargs)
				return mt(5, 'Energy has not fully recovered, successful energy update', {'energy': sql_energy, 'recover_time': recover_time, 'cooling_time': cooling_time})

		if recovered_energy + sql_energy >= max_energy:
			# 成功5：如果有恢复时间且是消耗能量
			# 满足上限的情况是用上限能量值减去要消耗的能量值，然后设置减去之后的能量值和当前的时间分别存入能量值项和恢复时间项
			sql_energy = max_energy - amount
			cooling_time = _cooling
			await execute_update(f'UPDATE progress SET energy = {sql_energy} WHERE uid = "{uid}";', **kwargs)
			...
			return mt(6, 'After refreshing the energy, the energy value and recovery time are successfully updated', {'energy': sql_energy, 'recover_time': current_time, 'cooling_time': cooling_time})
		elif recovered_energy + sql_energy - amount >= 0:
			# 成功6：如果有恢复时间且是消耗能量
			# 不满足上限的情况是用当前数据库的能量值和当前恢复的能量值相加然后减去消耗的能量值为要存入数据库的能量值项
			# 数据库中的恢复时间与恢复能量消耗的时间相减的恢复时间值存入到数据库的恢复时间项
			sql_energy = recovered_energy + sql_energy - amount
			recover_time = (datetime.strptime(recover_time, '%Y-%m-%d %H:%M:%S').replace(tzinfo=TZ_SH) + timedelta(seconds=recovered_energy * _cooling)).strftime("%Y-%m-%d %H:%M:%S")
			...
			cooling_time = _cooling - int(delta_time.total_seconds())
			await execute_update(f'UPDATE progress SET energy = {sql_energy} WHERE uid = "{uid}";', **kwargs)
			await execute_update(f'UPDATE timer SET time = "{recover_time}" WHERE uid = "{uid}" AND tid = "{enums.Timer.ENERGY_RECOVER_TIME.value}";', **kwargs)
			return mt(7, 'Energy has been refreshed, not fully recovered, energy has been consumed, energy value and recovery time updated successfully', {'energy': sql_energy, 'recover_time': recover_time, 'cooling_time': cooling_time})
