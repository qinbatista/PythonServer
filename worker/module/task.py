...
async def record(uid, tid, **kwargs):
    ...
    await set_task(uid, tid, time=_now, **kwargs)
    await common.execute(f'INSERT INTO task (uid, tid, value, reward, timer) SELECT "{uid}", {enums.Task.DONE_10_TASK}, IF((@i:=COUNT(*)) >= 10, 1, 0), 0, IF(COUNT(*) >= 10, "{_now}", "") FROM task WHERE uid = "{uid}" ON DUPLICATE KEY UPDATE value=IF(@i >= 11 AND reward != 1, 1, value), timer=IF(@i >= 11 AND reward != 1 AND timer = "", "{_now}", timer);', **kwargs)
...
async def get_task_reward(uid, tid, **kwargs):
    ...
    if data[0][0] == 1 and data[0][1] == 0:
        ...
        await common.execute(f'UPDATE task set reward = 1 WHERE uid = "{uid}" AND tid = "{tid}";', **kwargs)
...
async def set_task(uid, tid, value=1, time=datetime.now(tz=common.TZ_SH).strftime("%Y-%m-%d %H:%M:%S"), **kwargs):
    await common.execute(f'INSERT INTO task (uid, tid, value,reward,timer) VALUES ("{uid}", {tid}, 1, 0, "{time}") ON DUPLICATE KEY UPDATE `value`= {value};', **kwargs)
