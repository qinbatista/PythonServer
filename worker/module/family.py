...
async def get_store(**kwargs):
    return common.mt(0, 'success', kwargs['config']['family']['store']['goods'])

async def purchase(uid, sid, **kwargs):
    in_family, name = await _in_family(uid, **kwargs)
    if not in_family: return common.mt(99, 'not in family')
    cfg = kwargs['config']['family']['store']['goods'].get(sid)
    if cfg is None:
        return common.mt(97, "goods doesn't exist")
    consume = list(cfg.keys())[0]
    can, cmw = await common.consume_items(uid, consume, **kwargs)
    if not can: return common.mt(98, 'lack of material')
    rm, rw = common.rm_rw(cmw)
    gn = await common.get_gn(uid, **kwargs)
...
async def welfare(uid, sid, **kwargs):
    in_family, name = await _in_family(uid, **kwargs)
    if not in_family: return common.mt(99, 'not in family')
    members = await common.execute(f'SELECT uid, gn FROM `player` WHERE `fid` = "{name}";', **kwargs)
    cfg = kwargs['config']['family']['store']['gift'].get(sid)
    if cfg is None:
        return common.mt(97, "gift doesn't exist")
    consume = list(cfg.keys())[0]
    can, cmw = await common.consume_items(uid, consume, **kwargs)
    if not can: return common.mt(98, 'lack of material')
    rm, rw = common.rm_rw(cmw)
