...
async def supplement_check_in(uid, **kwargs):
	...
	isok, diamond = await common.try_item(uid, enums.Item.DIAMOND, -missing_days * kwargs["check_in_config"]["patch_diamond"], **kwargs)
	if not isok: return common.mt(99, 'Insufficient diamond')
	check_data = {}
	...
	return common.mt(0, 'Successful signing', data={'supplement': check_data, 'remaining': {'diamond': diamond},
													'reward': {'diamond': missing_days * kwargs["check_in_config"][
														"patch_diamond"]}})
...
async def check_in(uid, **kwargs):
	...
	current_time = time.strftime('%Y-%m-' + str(kwargs["hard_day"]), time.localtime()) if kwargs.__contains__(
		"hard_day") else time.strftime('%Y-%m-%d', time.localtime())
	...
	item_set = kwargs["check_in_config"][str(which_day)].split(":")
