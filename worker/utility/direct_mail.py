...
async def send_verification(to, nonce, session, status=0):
	if status == 0:
		BASE_REQUEST['Subject'] = '陆逊互娱账户注册验证码'
		body = VERIFY_TEMPLATE.replace('~NONCE~', nonce).replace('~SUBJECT~', '感谢您注册陆逊互娱账号！')
	else:
		BASE_REQUEST['Subject'] = '解绑陆逊互娱账户验证码'
		body = VERIFY_TEMPLATE.replace('~NONCE~', nonce).replace('~SUBJECT~', '你正在解绑你的邮箱！解绑邮箱后，可能会存在账号丢失风险！')
	snonce = str(secrets.randbits(65))
	tstamp = datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
	cqs = canonicalized_query_string(**BASE_REQUEST, HtmlBody=body, SignatureNonce=snonce, Timestamp=tstamp, ToAddress=to)
	signature = sign(cqs)
	async with session.post(ALIYUN_DIRECT_MAIL_API, params=merge_kwargs(**BASE_REQUEST, HtmlBody=body, SignatureNonce=snonce, Timestamp=tstamp, ToAddress=to, Signature=signature)) as r:
		return 'OK' if r.status == 200 else await r.text()
