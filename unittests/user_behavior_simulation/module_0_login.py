...
def login_account():  # 账户登陆
	...
	if len(world_list) != 0:
		...
		world = enter_world(token, "s1")  # 选择世界,游客登陆需要返回一个用户不是很忙的服务器,目前只有世界0,未完成
...
if __name__ == "__main__":
	# login_module("0")
	token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE1Nzg1NzA3MDMsInVpZCI6InEzIn0.mRIb5SIFZqfCxVUSvx4sppfOGebSu5Q0BWtHu3d9EuA"
	user_behavior_simulation.send_tcp_message({'function': 'bind_email', 'data': {"token": token, "email": "2428437133@qq.com"}})
	code = input("code:")
	user_behavior_simulation.send_tcp_message({'function': 'verify_email_code', 'data': {"token": token, "code": code}})
	# user_behavior_simulation.send_tcp_message({'function': 'bind_phone', 'data': {"token": token, "phone_number": "18323019610"}})
	# code = input("code:")
	# user_behavior_simulation.send_tcp_message({'function': 'verify_phone_code', 'data': {"token": token, "code": code}})
