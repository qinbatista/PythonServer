...
def request_join_family(_token,_world):
	response = send_tcp_message({'world' : 0, 'function' : 'request_join_family', 'data' : {'token': _token, 'name': '广告歌'}})
	print_method(str(response))
...
def set_role_family(_token,_world):
	user_behavior_simulation.send_tcp_message({'world' : 0, 'function' : 'set_role_family', 'data' : {'token': _token, 'gn_target':"去污","role":4}})
	print_method(str(response))
...
def family_dialog(token,world,info,unique_id):
	while True:
		...
		if choice=="5": user_behavior_simulation.send_tcp_message({'world' : world, 'function' : 'request_join_family', 'data' : {'token' : token, "name":"21"}})
		...
		if choice=="9": user_behavior_simulation.send_tcp_message({'world' : world, 'function' : 'market_purchase_family', 'data' : {'token' : token, "item":"3:6:2"}})
		...
		if choice=="15":user_behavior_simulation.send_tcp_message({'world' : world, 'function' : 'change_name_family', 'data' : {'token' : token, "name":"新家族名字"}})
	...
	# remove_user_family(_token,_world)
	user_behavior_simulation.send_tcp_message({'world' : world, 'function' : 'request_join_family', 'data' : {'token' : token, "name":"广告歌"}})
...
if __name__ == '__main__':
	...
	tk1 = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE1NzkyNDU5ODUsInVpZCI6ImgwMDEifQ.sTgtp0v9k9K-316Ey-lVTjacszUqbArD6kpQC65NRRw'
	tk2 = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE1Nzg3MjgxMzEsInVpZCI6ImgwMDAifQ.C_CRmyMSK8a1MxW74UfwzmfraVJ74XYt2wqYkn0xgo4'
	family_dialog(tk1, 's1', '', '')
	# invite_user_family()
	pass
