...
@login_decoration
def get_all_market(world, token, **kwargs):
	response = send_tcp_message({'world': world, 'function': 'get_all_market', 'data': {'token' : token}})
	logger.debug(response)

@login_decoration
def refresh_market(world, token, **kwargs):
	response = send_tcp_message({'world': world, 'function': 'refresh_market', 'data': {'token' : token}})
	logger.debug(response)

@login_decoration
def darkmarket_transaction(world, token, pid, **kwargs):
	response = send_tcp_message({'world': world, 'function': 'darkmarket_transaction', 'data': {'token' : token, 'pid': pid}})
	logger.debug(response)

def darkmarket_dialog(world, token, pid, **kwargs):
	get_all_market(world, token, **kwargs)
	refresh_market(world, token, **kwargs)
	darkmarket_transaction(world, token, pid, **kwargs)
