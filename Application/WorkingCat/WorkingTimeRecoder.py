...
class WorkingTimeRecoderClass():
	...
	def _get_someday_information(self, message_info):
		...
		sql = "SELECT t.unique_id, t.check_in, t.check_out, t.data_time " + "FROM timeinfo t " + "WHERE t.data_time ='" + date + "' AND t.unique_id = '" + session + "';"
		staff = wcsql(sql)
		print("wcsql->data:" + str(staff))
		if len(staff) == 0: return MessageList[7] % ("None", "None")
		return MessageList[7] % (staff[0][1], ("None" if staff[0][2] is None else staff[0][2]))
