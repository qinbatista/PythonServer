...
class BagSystemClass():
	...
	def __increase_item_quantity(self,item_id, item_quantity):
		try:
	...
	def _increase_supplies(self,message_info):
		...
		if(int_number==1):
			...
			if skill_level_result>0:
				#增加卷轴
				return_dic.update (self.__increase_item_quantity("m1_level","1"))
		if "scroll_skill_10" in message_dic["data"].keys():
			scroll_quantity = message_dic["data"]["scroll_skill_10"]
			return_dic.update (self.__increase_item_quantity("scroll_skill_10",scroll_quantity))
		if "scroll_skill_30" in message_dic["data"].keys():
			scroll_quantity = message_dic["data"]["scroll_skill_30"]
			return_dic.update (self.__increase_item_quantity("scroll_skill_30",scroll_quantity))
		if "scroll_skill_100" in message_dic["data"].keys():
			scroll_quantity = message_dic["data"]["scroll_skill_100"]
			return_dic.update (self.__increase_item_quantity("scroll_skill_100",scroll_quantity))
		if "iron" in message_dic["data"].keys():
			scroll_quantity = message_dic["data"]["iron"]
			return_dic.update (self.__increase_item_quantity("iron",scroll_quantity))
		if "diamonds" in message_dic["data"].keys():
			scroll_quantity = message_dic["data"]["diamonds"]
			return_dic.update (self.__increase_item_quantity("diamonds",scroll_quantity))
		if "money" in message_dic["data"].keys():
			scroll_quantity = message_dic["data"]["money"]
			return_dic.update (self.__increase_item_quantity("money",scroll_quantity))
		if "coin" in message_dic["data"].keys():
			scroll_quantity = message_dic["data"]["coin"]
			return_dic.update (self.__increase_item_quantity("coin",scroll_quantity))
