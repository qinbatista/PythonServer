...
class BagManager:
	...
	async def get_small_energy_potion(self, unique_id: str):
		potions = await self._execute_statement("SELECT small_energy_potion FROM bag WHERE unique_id='" + str(unique_id) + "';")
		if () in potions or potions == None:
			return 0
		return potions[0][0]
	...
	async def _update_quantity(self, unique_id: str, column_name: str, quantity: int):
		await self._execute_statement("UPDATE bag SET `" + str(column_name) + "`='" + str(quantity) + "' WHERE unique_id='" + str(unique_id) + "';")
